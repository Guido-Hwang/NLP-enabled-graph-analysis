...
def test(args):
    ...
    device = torch.device('cuda' if args['--cuda'] else 'cpu')
    model = bilstm_crf.BiLSTMCRF.load(args['MODEL'], device) # 数据装载错了
    print('start testing...')
    print('using device', device)

    result_file = open(args['RESULT'], encoding='utf-8', mode='w')
    model.eval()
    with torch.no_grad():
        for sentences, tags in tqdm(utils.batch_iter(test_data, batch_size=int(args['--batch-size']), shuffle=False)):
            ...
            for sent, true_tags, pred_tags in zip(sentences, tags, predicted_tags):  # tags 越界
...
if __name__ == '__main__':
    ...
    args_dict = {'--dropout-rate': 0.5,
                 ...
                 '--model-save-path': './model/guido_model.pth',
                 '--optimizer-save-path': './model/guido_optimizer.pth',
                 '--cuda': False}
    if is_train:
        args_dict.update({'TRAIN': './data/guido_train.txt',
                          'SENT_VOCAB': './vocab/guido_sent_vocab.json',
                          'TAG_VOCAB': './vocab/guido_tag_vocab.json',
                          '--batch-size': 512,
                          })
        train(args_dict)
    else:
        args_dict.update({'TEST': './data/guido_test.txt',
                          'RESULT': './guido_result_test.txt',
                          'SENT_VOCAB': './vocab/guido_sent_vocab.json',
                          'TAG_VOCAB': './vocab/guido_tag_vocab.json',
                          'MODEL': './model/guido_model.pth',
                          '--batch-size': 256
                          })
        test(args_dict)

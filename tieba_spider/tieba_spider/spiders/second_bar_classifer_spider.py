...
class BarClassiferSpider(scrapy.Spider):
    ...
    将用户的关注的贴吧分为耳机目录
    '''
    name = 'bar_classifer_second_spider'
    ...
    def __init__(self):
        ...
        self.floor = 1
        tieba_spider.csv_file_name = f'user_L{self.floor}_bar_classify'
        try:
            self.user_pd = pd.read_csv(f'{tieba_spider.data_path}{tieba_spider.csv_file_name}_classify_second.csv',
                                       encoding='utf_8_sig')
            print('进行查漏补缺')
            print(self.user_pd.shape[0])
            print(f"还有{self.user_pd[self.user_pd['forum_category_second'].isnull()].shape[0]}个贴吧二级目录，因为某种原因未能归类")
        except BaseException as error:
            print(error)
            self.user_pd = pd.read_csv(f'{tieba_spider.data_path}{tieba_spider.csv_file_name}.csv',
                                       encoding='utf_8_sig',
                                       engine='python')
            self.user_pd['forum_category_second'] = None
        else:
            print(f"有{self.user_pd[self.user_pd['bar_name'].isnull()].shape[0]}没有贴吧二级目录的行,开始过滤掉这些行")
            self.user_pd.dropna(subset=['bar_name'], inplace=True)

        # 将信号与函数绑定
        dispatcher.connect(self.spider_closed, signals.spider_closed)
        # 读取json文件内容,返回字典格式
        with open('bar_type_second.json', 'r', encoding='utf8')as fp:
    ...
    def spider_closed(self, spider):
        print(f"还有{self.user_pd[self.user_pd['forum_category_second'].isnull()].shape[0]}个贴吧，因为某种原因未能归类")
        print(self.user_pd.shape[0])
        print(self.user_pd[self.user_pd['forum_category_second'].isnull()].shape[0])
        self.user_pd.to_csv(f"{tieba_spider.data_path}{tieba_spider.csv_file_name}_classify_second.csv",
                            encoding='utf_8_sig', index=False)
        with open('bar_type_second.json', 'w', encoding='utf8')as fp:
            json.dump(tieba_spider.bar_type, fp, ensure_ascii=False)

    def parse(self, response):
        for index, rows in self.user_pd[self.user_pd['forum_category_second'].isnull()].iterrows():  ### 再次爬取，上次爬取有丢失
            ...
            try:
                ...
                if forum not in tieba_spider.bar_type.keys():  # 如果之前没有查询过
                    ...
                    self.user_pd.at[index, 'forum_category_second'] = tieba_spider.bar_type[forum]  # 字典没有问题
                if not index % 1000:
    ...
    def parse_bar_category(self, response):
        ...
        try:
            ...
            self.user_pd.at[index, 'forum_category_second'] = bar_category
...
if __name__ == '__main__':
    ...
    process.crawl('bar_classifer_second_spider')
    process.start()
